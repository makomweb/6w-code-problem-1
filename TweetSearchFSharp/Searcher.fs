...
let create_basic_auth(key:string, secret:string) = 
...
let deserialize_credentials(raw:string) =
...
let deserialize_tweets(raw:string) =
    let resp = Json.JsonConvert.DeserializeObject<search_response>(raw)
    resp.statuses

let get_tweets(search_term:string) =
    async {
        let http = new HttpClient()
